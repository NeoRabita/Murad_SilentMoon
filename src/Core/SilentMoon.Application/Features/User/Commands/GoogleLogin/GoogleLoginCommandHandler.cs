...
namespace SilentMoon.Application.Features.User.Commands.GoogleLogin
{
    public class GoogleLoginCommandHandler : ICommandHandler<GoogleLoginCommand, LoginResponse>
    {
        ...
        public GoogleLoginCommandHandler(IUow uow,IGoogleAuthService google,ITokenGeneratorService token)
        {
            _uow = uow;
            _google = google;
            _token = token;
        ...
        public async Task<Result<LoginResponse>> Handle(GoogleLoginCommand command, CancellationToken ct)
        {
            ...
            if (googleUser == null)
            {
                return Error.Unauthorized("Google","Invalid token");
            }

            var repo = _uow.GetRepository<ApplicationUser>();
            ...
            var accessToken = await _token.GenerateJwtAccessTokenAsync(user);

            var (refreshToken, refreshTokenExpires) = await _token.GenerateRefreshTokenAsync();
